  ...
  <div class="q-pa-md">
    ...
    >
      ...
      <template v-slot:body-cell-id="props">
        <q-td
          :props="props"
          @click="goToacademicMajorPage(props.row.id)"
    ...
    <div style="display: flex; justify-content: flex-end; margin-top: 20px">
      <q-btn
        ...
        :label="$t('add_genre')"
        class="button"
        to="/admin/academic_majors/create"
      />
      <q-btn
        unelevated
        color="negative"
...
export default {
  ...
  methods: {
    ...
    deleteAcademicMajors() {
      const confirmation =
        this.$i18n.locale === "ru"
          ? confirm("Вы уверены, что хотите удалить выбранные специальности?")
          : confirm("Are you sure you want to delete theese academic majors?");
      if (!confirmation) return;
      this.selected.forEach(async (tag) => {
        try {
          const response = await fetch(
            serverAdress + "/info/academic_majors/" + tag.id,
            {
              method: "DELETE",
            }
          );
  ...
  async mounted() {
    this.rows = this.getAllAcademicMajorsRows(
      await this.getAllAcademicMajors()
    );
  },
};
</script>
