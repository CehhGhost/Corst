  ...
  <div class="q-pa-md">
    ...
    >
      ...
      <template v-slot:body-cell-id="props">
        <q-td
          :props="props"
          @click="goToCoursePage(props.row.id)"
...
export default {
  ...
  methods: {
    ...
    deleteCourses() {
      const confirmation =
        this.$i18n.locale === "ru"
          ? confirm("Вы уверены, что хотите удалить выбранные курсы?")
          : confirm("Are you sure you want to delete theese courses?");
      if (!confirmation) return;
      this.selected.forEach(async (tag) => {
        try {
          const response = await fetch(
            serverAdress + "/info/courses/" + tag.id,
            {
              method: "DELETE",
            }
          );
  ...
  async mounted() {
    this.rows = this.getAllCoursesRows(await this.getAllCourses());
  },
};
</script>
